...
const app = express()
const { baseWebhookURL, sessionFolderPath, maxAttachmentSize, setMessagesAsSeen, webVersion, webVersionCacheType, recoverSessions, webSocketPort } = require('./config')
const { emitWebSocket, triggerWebhook, waitForNestedObject, checkIfEventisEnabled, initializeWebSocket } = require('./utils')

let socket
if (webSocketPort) {
  socket = initializeWebSocket(app, webSocketPort)
} else {
  socket = undefined
}
